...
public class BNode<T extends Comparable<T>> {
	...
	private void reorganizador(LinkedList<BNode<T>> filhos, BNode<T> node, int first, int last) {
		...
		int index = first;
		while (index < last) {
			aux = position(node.getElements(), filhos.get(index).getElements().get(0));
			node.addChild(aux, filhos.get(index));
			index++;
		}
	}
	...
	protected void split() {
		T meio = getElements().get(getElements().size() / 2);
		int aux, left, right;
		...
		guardaElementos(meio, maior, menor);
		if (getParent() == null && !isLeaf()) {
			...
			node.getElements().add(meio);
			node.parent = getParent();

			menor.parent = getParent();
			maior.parent = getParent();
			aux = position(node.getParent().getElements(), meio);
			left = aux;
			right = aux + 1;
			...
			menor.parent = getParent();
			maior.parent = getParent();
			aux = position(node.getElements(), meio);
			left = aux;
			right = aux + 1;
